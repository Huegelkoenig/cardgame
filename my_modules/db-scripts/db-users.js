...
function validateCredentials(req, res){
  ...
  pool.query(`SELECT ${PASSWORD} FROM ${TABLE} WHERE ${USERNAME} = ?;`, req.body.loginusername, (err, sqlResult)=>{
    if (err){
      new Status({status:'error', file:'db-users.js', func: 'validateCredentials()', line: 139/*LL*/, date:DateToString(new Date()), msg: `pool.query() threw an error`, error: err})
    ...
    bcrypt.compare(req.body.loginpassword, sqlResult[0][PASSWORD], (err,match)=>{
      if (err){
        new Status({status:'error', file:'db-users.js', func: 'validateCredentials()', line: 155/*LL*/, date:DateToString(new Date()), msg: `pool.query() threw an error`, error: err})
...
function loginResponse(req, res, $_oldToken=undefined){
  ...
  pool.query(`UPDATE ${TABLE} SET ${SESSIONID} = ? WHERE ${USERNAME} = ?;`, [sessionID, username], (err,sqlResult)=>{
    if (err){
      new Status({status:'error', file:'db-users.js', func: 'loginResponse()', line: 193/*LL*/, date:DateToString(new Date()), msg: `pool.query() threw an error`, error: err})
...
function validateSessionID(socket){
  return new Promise((resolve, reject)=>{
    ...
    pool.query(`SELECT ${SESSIONID} FROM ${TABLE} WHERE ${USERNAME} = ?;`, socket.handshake.query.username, (err, sqlResult)=>{
      if (err){
        reject(new Status({status:'error', file:'db-users.js', func: 'validateSessionID()', line: 237/*LL*/, date:DateToString(new Date()), msg: `pool.query() threw an error`, usermsg: `Oups, something went wrong. Maybe the server is down. Error-Code "DB-U:${237/*LL*/}"`, error: err}));
        return;
      }
      ...
      pool.query(`UPDATE ${TABLE} SET ${SESSIONID} = ? WHERE ${USERNAME} = ?;`, ['', socket.handshake.query.username], (err,sqlResult)=>{
        if (err){
          reject(new Status({status:'error', file:'db-users.js', func: 'validateSessionID()', line: 253/*LL*/, date:DateToString(new Date()), msg: `pool.query() threw an error`, usermsg: `Oups, something went wrong. Maybe the server is down. Error-Code "DB-U:${253/*LL*/}"`, error: err}));
          return;
        }
        resolve(true);
      });
...
function recoverCredentials(req,res){
  ...
  pool.query(`SELECT ${USERNAME} FROM ${TABLE} WHERE ${EMAIL} = ?;`, req.body.recoveryMail, (err, sqlResult)=>{
    if (err){
      new Status({status:'error', file:'db-users.js', func: 'recoverCredentials()', line: 297/*LL*/, date:DateToString(new Date()), msg: `pool.query() threw an error`, error: err})
    ...
    sqlResult.forEach((result)=>{
      ...
      promises.push(new Promise((resolve,reject)=>{
        ...
        pool.query(`UPDATE ${TABLE} SET ${RECOVERYTOKEN} = ?, ${RECTOKENEXP} = ? WHERE ${USERNAME} = ?;`, [recoverID, Date.now()+60*60*1000, recoverUsername], (err,sqlResult)=>{
          if (err){
            reject(new Status({status:'error', file:'db-users.js', func: 'recoverCredentials()', line: 313/*LL*/, date:DateToString(new Date()), msg: `pool.query() threw an error`, usermsg: `Oups, something went wrong. Maybe the server is down. Error-Code "DB-U:${313/*LL*/}"`, error: err}));
            return;
          }
          //create email
          let mailOptions = {
          ...
          transporter.sendMail(mailOptions, (err,info)=>{
            if (err){
              reject(new Status({status:'error', file:'db-users.js', func: 'recoverCredentials()', line: 331/*LL*/, date:DateToString(new Date()), msg: `transporter.sendMail threw an error`, usermsg: `Oups, something went wrong. Couldn't sent emails. Error-Code "DB-U:${331/*LL*/}"`, error: err}));
              return;
            }
            resolve(true);
          });
...
function resetPassword(req,res){
  ...
  if (!name || !password || !passwordconfirmation || !resetID){ //insufficient credentials (must be wrong usage of POSTMAN ;-D), sending reset.html again
    ...
    pool.query(`SELECT ${RECTOKENEXP} FROM ${TABLE} WHERE ${USERNAME} = ? AND ${RECOVERYTOKEN} = ?;`, [name, resetID], (err, sqlResult)=>{
      if (err){
        new Status({status:'error', file:'db-users.js', func: 'resetPassword()', line: 403/*LL*/, date:DateToString(new Date()), msg: `pool.query() threw an error`, error: err})
      ...
      bcrypt.hash(password, 10, (err,hash)=>{
        if (err){
          new Status({status:'error', file:'db-users.js', func: 'resetPassword()', line: 419/*LL*/, date:DateToString(new Date()), msg: `bcrypt.hash() threw an error`, error: err})
...
function registerUser(req,res){
  ...
  // 2) try to register the user
    bcrypt.hash(password, 10, (err,hash)=>{//generate a salt, afterwards hash the password with 10 rounds
      if (err){
        new Status({status:'error', file:'db-users.js', func: 'registerUser()', line: 520/*LL*/, date:DateToString(new Date()), msg: `bcrypt.hash() threw an error`, error: err})
      ...
      pool.query(`INSERT INTO ${TABLE} (${USERNAME}, ${PASSWORD}, ${EMAIL}) VALUES (?, ?, ?);`, [name, hash, email], (err,sqlResult)=>{
        if (err){
          ...
          new Status({status:'error', file:'db-users.js', func: 'registerUser()', part: '2) try to register the user', line: 534/*LL*/, date:DateToString(new Date()), msg: `pool.query threw an error, see .error for details`, error: err})
